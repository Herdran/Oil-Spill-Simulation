...
def _project_coordinates(lat: float, lon: float, width: int, height: int) -> tuple[int, int]:
    x = _project_longitude_to_x(lon, width)
    y = _project_latitude_to_y(-lat, height)
    return x, y

def _project_to_coordinates_raw(x: int, y: int, width: int, height: int) -> tuple[float, float]:
...
def project_binary_map_coordinates_raw(lat: float, lon: float) -> tuple[float, float]:
...
def project_binary_map_xy_to_coordinates_raw(x: int, y: int) -> tuple[float, float]:
