...
class ImageViewer(tk.Canvas):
    def __init__(self, parent, simulation_engine, image_array_shape, image_change_controller, initial_zoom_level,
                 full_img):
    ...
    def on_button_release(self, event):
        ...
        if 0 <= x < self.image_array_width and 0 <= y < self.image_array_height:
            coord = (x, y)
            if self.simulation_engine.get_topography(coord) == simulation.TopographyState.SEA:
                if coord not in self.simulation_engine.world:
                    self.simulation_engine.world[coord] = simulation.Point(coord, self.simulation_engine)

                point_clicked = self.simulation_engine.world[coord]
    ...
    def on_motion(self, event):
        ...
        if not (0 <= self.tooltip_coord[0] < self.image_array_width and 0 <= self.tooltip_coord[
            1] < self.image_array_height):
            self.hide_tooltip()
            return
        self.show_tooltip(event.x_root, event.y_root)
    ...
    def update_tooltip_text(self):
        if self.tooltip:
            if self.tooltip_coord not in self.simulation_engine.world:
                text = f"Oil mass: {0: .2f}kg"
            else:
                tooltip_point = self.simulation_engine.world[self.tooltip_coord]
    ...
    def resize(self, event=None):
        ...
        self.initial_zoom_level = min(window_width / self.image_array_width, window_height / self.image_array_height)
        self.zoom_level *= self.initial_zoom_level
        self.update_image()
...
class ImageChangeController(tk.Frame):
    def __init__(self, parent, simulation_engine, full_img):
        super().__init__(parent)
        self.simulation_engine = simulation_engine
        ...
        self.infobox_current_iteration = create_label_grid(frame_infoboxes_values, "", 0, 0, sticky=tk.N + tk.S + tk.E,
                                                           anchor="e", justify="right")
        self.infobox_simulation_time = create_label_grid(frame_infoboxes_values, "", 1, 0, sticky=tk.N + tk.S + tk.E,
                                                         anchor="e", justify="right")
        self.infobox_global_oil_amount_sea = create_label_grid(frame_infoboxes_values, "", 2, 0,
                                                               sticky=tk.N + tk.S + tk.E, anchor="e", justify="right")
        self.infobox_global_oil_amount_land = create_label_grid(frame_infoboxes_values, "", 3, 0,
                                                                sticky=tk.N + tk.S + tk.E, anchor="e", justify="right")
        self.infobox_dispersed_oil = create_label_grid(frame_infoboxes_values, "", 4, 0, sticky=tk.N + tk.S + tk.E,
                                                       anchor="e", justify="right")
        self.infobox_evaporated_oil = create_label_grid(frame_infoboxes_values, "", 5, 0, sticky=tk.N + tk.S + tk.E,
                                                        anchor="e", justify="right")
        self.infobox_oil_area = create_label_grid(frame_infoboxes_values, "", 6, 0, sticky=tk.N + tk.S + tk.E,
                                                  anchor="e", justify="right")
        self.infobox_current_zoom = create_label_grid(frame_infoboxes_values, "1.0", 7, 0, sticky=tk.N + tk.S + tk.E,
                                                      anchor="e", justify="right")
    ...
    def wait_for_thread_to_unlock_buttons(self):
        if not self.thread_finished:
            ...
            if not self.simulation_engine.is_finished():
                self.btn_start_stop.config(state=NORMAL)

    def threaded_function(self):
        while self.is_running:
            self.simulation_engine.update(self.minimal_oil_to_show)
    ...
    def update_image_array(self, full_update=False):
        if self.update_occurred and self.is_running or full_update:
            points_changed, points_removed = (
                self.simulation_engine.points_changed, self.simulation_engine.points_removed) if not full_update else (
                self.simulation_engine.world, [])
            if not full_update:
                self.viewer.update_tooltip_text()

            for coords in points_changed:
                if coords in points_removed:
                    continue
                pixel_color = self.simulation_engine.world[coords].pixel_color
                self.full_img.putpixel((coords[0], coords[1]), pixel_color)

            for coords in points_removed:
                if coords in self.simulation_engine.lands:
        ...
        self.update_infobox()
        if self.simulation_engine.is_finished():
            if self.is_running:
    ...
    def update_infobox(self):
        ...
        self.infobox_simulation_time.configure(
            text=generate_string_for_displaying_time(self.simulation_engine.total_time))

        self.update_oil_amount_infobox()
        self.update_idletasks()

    def update_oil_amount_infobox(self):
        global_oil_amount_sea, global_oil_amount_land = self.simulation_engine.get_oil_amounts()
        global_oil_amount_sea += self.value_not_yet_processed

        self.infobox_global_oil_amount_sea.configure(
            text=generate_string_for_displaying_oil_amount(global_oil_amount_sea))
        self.infobox_global_oil_amount_land.configure(
            text=generate_string_for_displaying_oil_amount(global_oil_amount_land))

        self.infobox_dispersed_oil.configure(
            text=generate_string_for_displaying_oil_amount(self.simulation_engine.dispersed_oil))
        self.infobox_evaporated_oil.configure(
            text=generate_string_for_displaying_oil_amount(self.simulation_engine.evaporated_oil))
        self.infobox_oil_area.configure(
            text=f"{(len(self.simulation_engine.world) * InitialValues.point_side_size ** 2) / 10 ** 6} km2")
    ...
    def save_checkpoint(self):
        self.simulation_engine.save_checkpoint(True)
...
def get_data_processor() -> DataProcessor:
    ...
    if InitialValues.data_preprocessor_initial_timestamp is not None:
        InitialValues.simulation_initial_parameters.time.min = deepcopy(
            InitialValues.data_preprocessor_initial_timestamp)
...
def start_simulation(window, points=None, oil_sources=None):
    ...
    frame_controller = ImageChangeController(main_frame, simulation_engine, full_img)

    initial_zoom_level = 1
    viewer = ImageViewer(main_frame, simulation_engine, image_array.shape, frame_controller, initial_zoom_level,
                         full_img)
