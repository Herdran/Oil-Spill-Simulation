...
def __get_measurment_stations(ds):
    ...
    nodes = [MeasureNode(longitude_data[i], latitude_data[i])
...
def __get_water_data(ds, measurment_station_count, measurments_count):
    return [
        [WaterCurrentData(ds.variables['v'][t, i], ds.variables['u'][t, i])
         for i in range(measurment_station_count)]
        for t in range(measurments_count)
    ]
...
def get_nearest_station(stations, lat, lon):
    return min(range(len(stations)),
               key=lambda i: math.sqrt(
                   (stations[i].latitude - lat) ** 2 + ((stations[i].longitude - lon) ** 2))
               )
...
def get_data():
    ...
    water_data = __get_water_data(
        ds, len(measurment_stations), len(measurment_times))
